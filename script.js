...
function initStory(storyImages) {
  ...
  function initLocation() {
   ...
   // 텍스트 (줄바꿈 처리)
    $('#locationBusSubway').innerHTML = (l.bussubway || '').replace(/\n/g, '<br>');
    $('#locationParking').innerHTML = (l.parking || '').replace(/\n/g, '<br>');
    $('#locationMeal').innerHTML = (l.meal || '').replace(/\n/g, '<br>');

    $('#locationMapImg').src = 'images/location/1.png';

    $('#kakaoMapBtn').href = w.mapLinks.kakao || '#';
    $('#naverMapBtn').href = w.mapLinks.naver || '#';

    $('#locationMapImg').addEventListener('click', () => {openPhotoModal(['images/location/1.png'], 0);
    });

    // 복사 버튼 2개
    $('#copyVenueAddressBtn').addEventListener('click', () => {copyToClipboard(l.venueAddress, '웨딩홀 주소가 복사되었습니다');
    });

    $('#copyParkingAddressBtn').addEventListener('click', () => {copyToClipboard(l.parkingAddress, '주차장 주소가 복사되었습니다');
    });
  }
